...
std::vector<Position> AStar::ConstructPath(Node* destination)
{
    ...
    // 루프가 종료되면 path에는 반대 방햐의 경로 정보가 저장됨
...
bool AStar::IsDirectionBlocked(const Position& currentPosition, const Direction& direction, const std::vector<std::vector<int>>& grid) const
{
    return false;
}
