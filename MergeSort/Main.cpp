...
void MergeSort(int* array, int length, Comparer comparer = LessEqual)
{
	...
	// 분할된 왼쪽 배열에 값 복사 
	memcpy(leftArray, array, sizeof(int) * leftArrayLength); //Todo 무조건 이해해야함
	
	// 분할된 왼쪽 배열에 값 복사
	memcpy(rightArray, array + middle, sizeof(int) * rightArrayLength); //Todo 무조건 이해해야함
