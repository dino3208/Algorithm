...
void InsertionSort(
	int* array, 
	const int length,
	std::function<bool(int, int)> comparer = std::greater<int>()) // Todo: 이해 못함
{
	// 예외 처리
	if (length <= 1)
	{
	...
	// 배열을 순회
	// 두번째 원소부터 순회 -> 첫 번째 원소는 정렬됐다고 가정하고 시작
	for (int ix = 1;ix < length;++ix)
	{
		...
		while (jx>=0 && comparer(array[jx], keyValue)) // Todo: 이해 못함
		{
			// 오른쪽으로 밀기(이동)
			array[jx + 1] = array[jx];
...
int main()
{
	...
	InsertionSort(array, length, [](int a, int b) {return a < b;}); // Todo: 람다함수임. 이해 못함
